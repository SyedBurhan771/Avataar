...
export default function BoardTab({ project, onTaskStatusChange }) {
  ...
  const todoTasks        = subtasks.filter(t => t.status === 'todo');
  ...
  const formatDate = (dateStr) => {
    if (!dateStr) return '—';
    // Handle DDMMYY format from Sage X3 (e.g. "010422")
    if (/^\d{6}$/.test(dateStr)) {
  ...
  const getAvatar = (task) => {
    ...
    const name = task.personResponsible || task.assignee || '';
          ...
          <div className="max-h-[620px] overflow-y-auto p-4 space-y-3">
            {todoTasks.map(task => (
              <div
                ...
                onClick={() => handleMove(task.id || task.taskCode, 'todo')}
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500 mt-3">
                    ...
                    <span>{task.personResponsible || task.assignee || 'Unassigned'}</span>
                  </div>

                  <div className="flex items-center gap-2 text-xs text-gray-500 mt-2">
                    <Calendar className="w-3.5 h-3.5" />
                    <span>Due {formatDate(task.endDate || task.dueDate)}</span>
                  </div>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm">
          ...
          <div className="max-h-[620px] overflow-y-auto p-4 space-y-3">
            {inProgressTasks.map(task => (
              <div
                key={task.id || task.taskCode}
                onClick={() => handleMove(task.id || task.taskCode, 'in-progress')}
        ...
        <div className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm">
          ...
          <div className="max-h-[620px] overflow-y-auto p-4 space-y-3">
            {doneTasks.map(task => (
              <div
                key={task.id || task.taskCode}
                onClick={() => handleMove(task.id || task.taskCode, 'completed')}
