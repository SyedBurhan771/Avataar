...
function Dashboard() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Upcoming Milestones + Deadlines Grid */}
          ...
          <div className="space-y-3">
            {upcomingMilestones.map((milestone, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${milestone.status === 'on-track' ? 'bg-green-500' : milestone.status === 'in-progress' ? 'bg-blue-500' : 'bg-red-500'}`} />
                  <div>
                ...
                <div className="text-right">
                  ...
                  <span className={`text-xs font-semibold ${milestone.status === 'on-track' ? 'text-green-600' : milestone.status === 'in-progress' ? 'text-blue-600' : 'text-red-600'}`}>
                    {milestone.status === 'on-track' ? 'On Track' : milestone.status === 'in-progress' ? 'In Progress' : 'At Risk'}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {upcomingDeadlines.map((dl, idx) => (
              <div key={idx} className={`flex items-center justify-between p-3 rounded-lg ${dl.overdue ? 'bg-red-50 border border-red-200' : 'bg-gray-50'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${dl.overdue ? 'bg-red-500' : 'bg-orange-400'}`} />
                  <div>
                ...
                <div className="text-right">
                  ...
                  <span className={`text-xs font-semibold ${dl.overdue ? 'text-red-600' : 'text-orange-600'}`}>
                    {dl.overdue ? `${Math.abs(dl.daysLeft)}d overdue` : `${dl.daysLeft}d left`}
                  </span>
                </div>
              </div>
            ))}
